...
// 1. Generate Static Paths ONLY for 2-wheeler packages
export async function generateStaticParams() {
  return packages
    .filter(
      (pkg) =>
        pkg.category?.toLowerCase().trim() === '2 wheeler'
    )
    .map((pkg) => ({
...
// 2. Dynamic SEO Metadata for every 2-wheeler area page
export async function generateMetadata({
  ...
  const pkg = packages.find(
    (p) =>
      p.areaSlug === areaSlug &&
      p.category?.toLowerCase().trim() === '2 wheeler'
  );
  ...
  return {
    title:
      pkg.seoTitle ||
      `Best Bike & Two Wheeler Driving School in ${pkg.subArea} | DriveSeekho`,

    description:
      pkg.seoDescription ||
      `Join professional two-wheeler & gear bike driving classes in ${pkg.subArea}, ${pkg.city}. Expert trainers and doorstep pickup available.`,
    ...
    openGraph: {
      title:
        pkg.seoTitle ||
        `Best Bike Driving School in ${pkg.subArea} | DriveSeekho`,

      description:
        pkg.seoDescription ||
        `Professional bike training classes in ${pkg.subArea}.`,
...
export default async function DrivingSchoolDetail({
  ...
  // IMPORTANT: Only find 2-wheeler package
  const pkg = packages?.find(
    (p) =>
      p.areaSlug === areaSlug &&
      p.category?.toLowerCase().trim() === '2 wheeler'
    ...
    <main className="max-w-7xl w-full mx-auto bg-slate-50 min-h-screen relative pb-40 lg:pb-16 selection:bg-blue-600 selection:text-white">
      ...
      <div className="relative w-full h-[300px] md:h-[450px] bg-slate-900">
        <img
          src={
            pkg.image ||
            'https://placehold.co/1200x600/1e293b/94a3b8?text=Bike+Driving+School'
          }
          alt={pkg.title}
          ...
          <div className="lg:col-span-2 bg-white rounded-[2rem] md:rounded-[2.5rem] px-5 md:px-10 py-8 md:py-10 shadow-xl shadow-slate-200/50 border border-slate-100">
            ...
            <div className="flex items-center gap-4 bg-slate-50 p-4 md:p-5 rounded-2xl mb-8 border border-slate-100/80">
              ...
              <div>
                ...
                <p className="text-[15px] md:text-[17px] font-extrabold text-slate-900">
                  {pkg.subArea}, {pkg.city}
                </p>
              </div>
            </div>

            {/* Course Highlights */}
            <h2 className="text-[18px] md:text-[20px] font-black text-slate-900 mb-4 tracking-tight">
              Course Highlights
            </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 md:gap-4 mb-10">
              ...
              <div className="flex items-center gap-4 bg-slate-50/80 border border-slate-100 p-4 md:p-5 rounded-2xl md:col-span-2">
                ...
                <div>
                  ...
                  <p className="text-[14px] md:text-[16px] font-black text-slate-900">
                    {pkg.carName || 'Geared Bike & Scooty'}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 bg-slate-50/80 border border-slate-100 p-4 md:p-5 rounded-2xl">
                ...
                <div>
                  ...
                  <p className="text-[14px] md:text-[15px] font-black text-slate-900">
                    {pkg.trainingDays}
                  </p>
                </div>
              </div>
            ...
            <div className="border-t border-slate-100 pt-8">
              <h2 className="text-[18px] md:text-[20px] font-black text-slate-900 mb-5 tracking-tight">
              ...
              <div className="space-y-3">
                {pkg.faqs?.map((faq: any, index: number) => (
                  <details
                    key={index}
                    className="group bg-slate-50 border border-slate-100 rounded-2xl overflow-hidden [&_summary::-webkit-details-marker]:hidden transition-all"
                  >
                    <summary className="flex items-center justify-between cursor-pointer p-4 md:p-5 font-extrabold text-[13px] md:text-[14px] text-slate-900 select-none">
                      ...
                      <ChevronDown
                        size={18}
                        className="text-slate-400 group-open:rotate-180 transition-transform duration-300 shrink-0"
                      />
                    </summary>
          ...
          <div className="hidden lg:block lg:col-span-1 lg:sticky lg:top-8 bg-white rounded-[2.5rem] p-7 md:p-8 shadow-2xl shadow-slate-200/60 border border-slate-100 space-y-6">
            ...
            <div>
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-[2.5rem] font-black text-blue-600 tracking-tight leading-none">
                  {pkg.price}
                </span>
            ...
            <div className="border-t border-slate-100 pt-5 flex flex-col gap-3">
              ...
              <a
                href={`https://wa.me/${
                  pkg.whatsappNumber || '918368510626'
                }?text=Hi, I want to book the bike training package in ${
                  pkg.subArea
                }.`}
                target="_blank"
